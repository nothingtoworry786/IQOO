...
class OpenRouterProvider(AIProvider):
    """AI provider that uses the OpenRouter API."""
    ...
    async def generate(self, prompt: str, system: str | None = None) -> str:
        ...
        try:
            ...
            raise AIProviderError(
                f"OpenRouter API returned status {exc.response.status_code}: {exc.response.text}"
            ) from exc
        except httpx.RequestError as exc:
            logger.error("OpenRouter API request failed: %s", exc)
            raise AIProviderError(f"OpenRouter API request failed: {exc}") from exc
        except (KeyError, IndexError) as exc:
            logger.error("OpenRouter API unexpected response format: %s", exc)
            raise AIProviderError(
                f"Unexpected response format from OpenRouter API: {exc}"
            ) from exc
